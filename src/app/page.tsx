...
export default function Home() {
  return (
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-rose-50 via-white to-pink-50">
      {/* Responsive Navbar */}
      <nav className="max-w-7xl mx-auto p-4 sm:p-6 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-2">
          <div className="bg-rose-600 p-2 rounded-lg shadow-md">
            <Pill className="text-white w-6 h-6" />
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">MediTrack</h1>
        </div>
        <div className="flex items-center gap-4 sm:gap-6">
          <Link href="/login" className="text-sm font-medium text-slate-600 hover:text-rose-600 transition-colors">
            Login
          </Link>
          <Link href="/register" className="bg-rose-600 text-white px-5 py-2.5 rounded-full text-sm font-medium hover:bg-rose-700 transition-all shadow-lg shadow-rose-200">
            Get Started
          </Link>
        </div>
      </nav>
      ...
      <main className="max-w-5xl mx-auto px-6 py-12 sm:py-24 text-center">
        <span className="inline-block px-4 py-1.5 mb-6 text-xs sm:text-sm font-semibold tracking-wide text-rose-700 uppercase bg-rose-100 rounded-full">
        ...
        <h2 className="text-4xl sm:text-6xl font-black text-slate-900 mb-6 leading-[1.1]">
          ...
          <span className="text-rose-600">on schedule.</span>
        </h2>
...
function FeatureCard({ icon, title, desc }: { icon: React.ReactNode; title: string; desc: string }) {
  return (
    <div className="p-6 sm:p-8 bg-white/70 backdrop-blur-md rounded-3xl border border-rose-100 hover:border-rose-300 transition-all hover:shadow-2xl hover:-translate-y-1 group">
      <div className="mb-4 p-3 bg-white rounded-2xl w-fit shadow-sm border border-rose-50 group-hover:bg-rose-50 transition-colors">
        {icon}
      </div>
