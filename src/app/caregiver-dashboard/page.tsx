                  ...
                  return (
                    <div key={med.id} className={`flex flex-col md:flex-row justify-between items-start md:items-center p-6 rounded-3xl transition-all ${
                      ...
                      <form action={async () => {
                        ...
                        <button 
                          type="submit"
                          className={`w-full md:w-auto px-6 py-3 rounded-2xl font-black text-xs transition-all ${
                            med.is_taken 
                              ? 'bg-green-600 text-white shadow-md' 
                              : 'bg-white text-slate-600 border-2 border-slate-200 hover:border-blue-500 hover:text-blue-600'
                          }`}
                        >
                          {med.is_taken ? 'Verified' : 'Manual Toggle'}
                        </button>
                      </form>
                    </div>
                  );
