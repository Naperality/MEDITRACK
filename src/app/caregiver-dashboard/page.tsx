...
import { recordMedicationAction } from "@/app/actions/medication"; // Using the updated record action
...
export default async function CaregiverDashboard() {
  ...
  // FETCH: Join caregiver_patient -> profiles -> medications & medication_logs
  // This ensures we get everything in one request
        ...
        <div className="space-y-12">
          ...
          {links?.map((link: any) => (
            <div key={link.patient_id} className="bg-white border border-slate-200 rounded-[2.5rem] shadow-sm overflow-hidden flex flex-col lg:flex-row">
              ...
              {/* LEFT SIDE: Patient Info & Medications (70% width on desktop) */}
                    ...
                    <div key={med.id} className={`p-6 rounded-[2rem] border transition-all ${
                      ...
                      <div className="flex flex-col md:flex-row justify-between gap-4">
                        <div className="space-y-3">
                          ...
                          <div className="flex items-center gap-4 pt-2 text-[11px] font-bold text-slate-400 uppercase">
                             <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5" /> Start: {med.start_date}</span>
                             <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5" /> End: {med.end_date}</span>
                          </div>
                        </div>

                        <form action={async () => {
                          'use server';
                          // This action now marks as taken AND adds to history log
                          await recordMedicationAction(med.id, link.patient_id, med.name);
                        }}>
                          <button className={`w-full md:w-auto px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-tight transition-all ${
                            med.is_taken 
                              ? 'bg-green-600 text-white' 
                              : 'bg-white text-slate-600 border border-slate-200 hover:border-blue-500'
                          }`}>
                            {med.is_taken ? 'Confirmed' : 'Mark as Taken'}
                          </button>
                        </form>
                      </div>
                    </div>
              ...
              {/* RIGHT SIDE: Activity Logs (The "Sidebar" area) */}
              <div className="w-full lg:w-96 bg-slate-50/50 p-8">
                <div className="flex items-center gap-2 mb-6">
                  <History className="w-5 h-5 text-blue-600" />
                  <h4 className="font-black text-slate-800 text-sm uppercase tracking-tighter">Activity History</h4>
                </div>

                <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                  {link.profiles.medication_logs?.sort((a: any, b: any) => 
                    ...
                    <div key={log.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden group">
                      ...
                      <div className="flex justify-between items-start mb-1">
                        ...
                        <span className="text-[9px] font-black bg-green-100 text-green-700 px-1.5 py-0.5 rounded uppercase">
                          {log.status}
                        </span>
                      </div>
                  ...
                  {link.profiles.medication_logs?.length === 0 && (
          ...
          {(!links || links.length === 0) && (
            <div className="text-center py-32 bg-white rounded-[3rem] border-4 border-dashed border-slate-100">
               ...
               <h2 className="text-2xl font-black text-slate-800">No Patients Monitored</h2>
               <p className="text-slate-400 max-w-xs mx-auto mt-2">Link a patient using their email address to see their medication schedule and logs.</p>
            </div>
          )}
        </div>
      </div>
